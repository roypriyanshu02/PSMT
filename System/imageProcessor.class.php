...
class imageProcessor {
	...
	private function create($image, $name, $newname, $type, $size, $c_type, $level) {
		//$im_name = time().$name;
	...
	function runs($image,$transcode,$imn_name) {

		// settings
		$c_type = $transcode['c_type'];
		$level = $transcode['level'];
		...
		if(in_array($c_type, array('jpeg','jpg','JPG','JPEG','gif','GIF','png','PNG'))) { // jpeg, png, gif only
			if(in_array($im_type, $this->setting['file_type'])){
				if($level >= 0 && $level <= 9){
					...
					$result['data'] = $this->create($image, $im_name, $imn_name, $im_type, $im_size, $c_type, $level);
				}else{
					$result['status'] = 'error';
	...
	public function run($image) {
		...
		foreach ($this->setting['transcode'] as $transcode_name => $transcode_info) {
			$imn_name = $mfilename."-".$transcode_name;
			$result = $this->runs($image,$transcode_info,$imn_name);
			@chmod($this->setting['directory'].$imn_name.'.'.$transcode_info['c_type'], 0777);
			$this->result['files'][$transcode_name]=$result['data']['compressed']['name'];
